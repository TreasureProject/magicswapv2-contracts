...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    function swapExactTokensForTokens(
        ...
        address to,
        uint deadline
    ) public virtual override returns (uint[] memory amounts) {
    ...
    function _swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], from, UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            amountOutput = UniswapV2Library.getAmountOut(amountInput, reserveInput, reserveOutput, address(pair), factory);
            }
